...
def replace_symbols(text, lang="en"):
    ...
    """
    text = text.replace(";", ",")
    text = text.replace("-", " ") if lang not in ["ca", "ro"] else text.replace("-", "")
    text = text.replace(":", ",")
    if lang == "en":
...
print(">> romanian_cleaners LOADED")
def romanian_cleaners(text):
    """Pipeline pentru text în limba română, inclusiv procesarea diacriticelor și a simbolurilor specifice."""
    text = lowercase(text)
    # Înlocuire simboluri specifice pentru limba română
    text = replace_symbols(text, lang="ro")
    # Eliminare simboluri auxiliare
    text = remove_aux_symbols(text)
